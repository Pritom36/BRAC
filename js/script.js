document.addEventListener('DOMContentLoaded', function() {
    ...
    async function checkSession() {
        ...
        // Force reload users data to get latest changes
        await loadUsers();
        
        const user = users.users.find(u => u.username === currentSession.username);
        
        if (!user || !user.isActive || isDateExpired(user.expiryDate)) {
            showAlert('Session expired or account inactive', true);
            handleLogout();
            return;
        }

        if (user.deviceId !== currentSession.deviceId) {
            showAlert('Session invalidated - logged in from another device', true);
            handleLogout();
            return;
        }
    }
    ...
    async function handleLogout() {
        if (currentSession) {
            try {
                await fetch('api/public/auth.php', {
                    method: 'POST',
                    headers: {
    ...
    loginForm.addEventListener('submit', async function(e) {
        ...
        // Update server with login
        try {
            const response = await fetch('api/public/auth.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
